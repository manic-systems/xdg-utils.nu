#![expect(
    clippy::string_slice,
    reason = "the parsers slice at indices they just validated"
)]
#![expect(
    clippy::use_debug,
    reason = "upstream diagnostics format with Debug on purpose"
)]
#![expect(
    clippy::unwrap_in_result,
    reason = "unwraps guard invariants the D-Bus type system already checked"
)]
#![expect(
    clippy::unreachable,
    reason = "marks match arms excluded by prior validation"
)]
