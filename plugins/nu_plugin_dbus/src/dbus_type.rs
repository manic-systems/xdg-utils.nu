...
impl DbusType {
    ...
    pub fn parse(input: &str) -> Result<(Self, &str), String> {
        use self::DbusType::{Byte, Boolean, Int16, UInt16, Int32, UInt32, Int64, UInt64, Double, String, ObjectPath, Signature, Array, Variant};
        ...
        match input.chars().nth(0).unwrap() {
            ...
            '(' => {
                ...
                loop {
                    ...
                    if let Some(new_remainder) = remainder.strip_prefix(')') {
                        break Ok((Self::Struct(types), new_remainder));
                    }
            ...
            '{' => {
                ...
                if let Some(new_remainder) = val_remainder.strip_prefix('}') {
                    Ok((
                        Self::DictEntry(key_type.into(), val_type.into()),
                        new_remainder,
                    ))
                } else {
                    Err(format!(
    ...
    pub fn parse_all(mut input: &str) -> Result<Vec<Self>, String> {
        let mut out = vec![];
    ...
    pub fn stringify(&self) -> String {
        use self::DbusType::{Byte, Boolean, Int16, UInt16, Int32, UInt32, Int64, UInt64, Double, String, ObjectPath, Signature, Array, Struct, Variant, DictEntry};
        ...
        match self {
            ...
            Struct(types) => std::iter::once("(".to_owned())
                .chain(types.iter().map(Self::stringify))
...
fn parse_simple_type_remainder() -> Result<(), String> {
...
#[test]
fn parse_simple_invalid() {
    DbusType::parse("*").unwrap_err();
}
...
#[test]
#[expect(clippy::panic_in_result_fn, reason = "tests assert")]
fn parse_array_remainder() -> Result<(), String> {
...
#[test]
fn parse_array_unclosed() {
    DbusType::parse("a").unwrap_err();
}
...
#[test]
#[expect(clippy::panic_in_result_fn, reason = "tests assert")]
fn parse_struct_remainder() -> Result<(), String> {
...
#[test]
fn parse_struct_unclosed() {
    DbusType::parse("(ss").unwrap_err();
}
...
#[test]
#[expect(clippy::panic_in_result_fn, reason = "tests assert")]
fn parse_dict_entry_remainder() -> Result<(), String> {
