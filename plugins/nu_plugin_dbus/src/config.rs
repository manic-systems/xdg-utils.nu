...
pub enum DbusBusChoice {
   ...
   Bus(String),
   /// Connect to a non-bus D-Bus server at the given address (will not send
   /// Hello).
   Peer(String),
}
      ...
      for (name, value) in &call.named {
         match &*name.item {
            r#type @ ("session" | "system" | "started") => {
               if value.is_none() || value.as_ref().is_some_and(nu_protocol::Value::is_true) {
            ...
            "timeout" => {
               if let Some(value) = value {
                  let nanos = u64::try_from(value.as_duration()?).map_err(|_| {
