...
impl Pattern {
    pub fn new(pattern: &str, separator: Option<char>) -> Self {
        let mut tokens = vec![];
    ...
    pub fn is_match(&self, string: &str) -> bool {
                    ...
                    if search_str.starts_with(s) {
                        ...
                        match state {
                            ...
                            MatchState::ScanAhead { stop_at_separator } => {
                                ...
                                let at_separator = stop_at_separator
                                    && self
                                        .separator
                                        .is_some_and(|sep| search_str.starts_with(sep));
                                // Consume a char either way. On the separator,
                                // revert to precise mode.
        ...
        #[cfg(test)]
        #[expect(clippy::print_stdout, reason = "test-only trace")]
        {
            println!(
                "end, state={state:?}, search_str={search_str:?}, tokens={tokens:?}"
            );
        }
        if search_str.is_empty() {
...
#[test]
#[expect(clippy::too_many_lines, reason = "exhaustive constructor cases")]
fn pattern_new() {
    assert_eq!(
        Pattern::new("", Some('/')),
        Pattern {
...
#[test]
fn pattern_is_match_one_wildcard() {
    let pat = Pattern {
        separator: Some('.'),
        tokens: vec![
...
#[test]
fn pattern_is_match_one_wildcard_at_end() {
    let pat = Pattern {
        separator: Some('.'),
        tokens: vec![
            PatternToken::Exact("foo.".into()),
...
#[test]
fn pattern_is_match_one_wildcard_at_start() {
    let pat = Pattern {
        separator: Some('.'),
        tokens: vec![
            PatternToken::OneWildcard,
...
#[test]
fn pattern_is_match_one_wildcard_no_separator() {
    let pat = Pattern {
        separator: None,
        tokens: vec![
            PatternToken::Exact("foo.".into()),
...
#[test]
fn pattern_is_match_many_wildcard() {
    let pat = Pattern {
        separator: Some('.'),
        tokens: vec![
            PatternToken::Exact("foo.".into()),
...
#[test]
fn pattern_is_match_many_wildcard_at_end() {
    let pat = Pattern {
        separator: Some('.'),
        tokens: vec![
            PatternToken::Exact("foo.".into()),
...
#[test]
fn pattern_is_match_many_wildcard_at_start() {
    let pat = Pattern {
        separator: Some('.'),
        tokens: vec![
            PatternToken::ManyWildcard,
