...
def --env make_lazy_default [dir: string, basefile: string, mode: string] {
    ...
    let mimetypes = (
        open --raw ($dir | path join $basefile) | lines | each {|l|
    ...
    for MIME in $mimetypes {
        ...
        if $mode == "user" {
            let xdg_user_dir = if (not ($env.XDG_DATA_HOME? == null) and not ($env.XDG_DATA_HOME | is-empty)) { $env.XDG_DATA_HOME } else {
...
# Update submenu
def --env update_submenu [
    menu_file: string
    mode: string
    action: string
    desktop_files: list<string>
    directory_files: list<string>
    my_umask: string
] {
...
def --wrapped main [...args] {
    ...
    let kde_user_dir = if not ($kde_ver | is-empty) {
        try {
            ^$"kde($kde_ver)-config" --path apps | complete | get stdout | str trim | split row ":" | get 0? | default ""
        } catch { "" }
    } else { "" }
    mut kde_global_dir = if not ($kde_ver | is-empty) {
        try {
            ^$"kde($kde_ver)-config" --path apps | complete | get stdout | str trim | split row ":" | get 1? | default ""
        } catch { "" }
    } else { "" }
